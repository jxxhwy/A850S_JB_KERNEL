...
static int msm_otg_reset(struct usb_phy *phy)
{
...
#if defined(FEATURE_HSUSB_SET_SIGNALING_PARAM)
	{
		...
		u32 reg = 0;
		/* set DC voltage level */
		reg = ulpi_read(phy, 0x81);
		printk(KERN_ERR "HS DC Level pre-value:0x%x\n", reg);
			reg = 0x3f;
		ulpi_write(phy, reg, 0x81);
		reg = ulpi_read(phy, 0x81);
			
		printk(KERN_ERR "HS DC Level post-value:0x%x\n", reg);

		
		/*set the Rise/Fall time value */
			reg = 0x3f; // -10%
		printk(KERN_ERR "initial HS TX Rise/Fall time  set value:0x%x\n", reg);
		ulpi_write(phy, reg, 0x82);
		reg = ulpi_read(phy, 0x82);
		printk(KERN_ERR "confirm HS TX Rise/Fall time  value:0x%x\n", reg);
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			work = 1;
#ifdef FEATURE_HSUSB_SET_SIGNALING_PARAM
			if(is_pantech_otg_enabled == 1){
				u32 reg = 0;
